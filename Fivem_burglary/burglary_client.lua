...
------ l o c a l e s ------
local noCar = "Sem carro perto"
local text = "~r~Usar~w~ Gazua?" -- lockpick the door
local textUnlock = "~g~[E]~w~ Entrar" -- enter the house
local insideText = "~g~[E]~w~ sair" -- exit the door
local abortConfirm = "Cancelaste a abertura forçada"
local searchText = "~g~[E]~w~ Procurar" -- search the spot
local emptyMessage = "Não há nada aqui!" -- if you press E where it is empty
local emptyMessage3D = "~r~Vazio" -- if the spot is empty
local closetText = "~g~[E]~w~ Espreitar para o armário" -- text at closet
local abortLock = "~g~[E]~w~ Para cancelar a abertura forçada"
local noLockpickText = "Não tens nenhuma gazua!" -- if you don't have a lockpick and you try to do the burglary
local carUnlocked = "Destrancaste o carro"
local youFound = "Na" -- when you steal something
local burglaryDetected = "Um assaltante foi detetado em" -- text 1 cops gets sent
local sentPhoto = "Nós enviamos uma foto do criminoso." -- if you use qalle's camerasystem this will be in the message too
...
local burglaryPlaces = {
  ...
  ["Covenant Avenue"] = {
    door = 1,
    locked = true,
    pos = { x = 171.44, y = -1871.48, z = 24.4, h = 247.05 }, -- door coords
    inside = { x = 346.52 , y = -1013.19 , z = -99.2, h = 357.81 }, -- Inside coords
    animPos = { x = 171.44, y = -1871.48, z = 24.4, h = 247.05 }, -- The animation position
    doorTime = {}
  },
  ["Grove Street 1"] = {
    door = 2,
    locked = true,
    pos = { x = 324.25, y = -1937.49, z = 25.02, h = 318.41},  -- door coords
    inside = { x = 346.52 , y = -1013.19 , z = -99.2, h = 357.81 },  -- Inside the house coords
    animPos = { x = 324.25, y = -1937.49, z = 25.02, h = 318.41 }, -- The animation position
    doorTime = {}
  }, 
  ["Grove Street 2"] = {
    door = 3,
    locked = true,
    pos = { x = 76.52 , y = -1948.16 , z = 21.17, h = 21.17 },  -- door coords
    inside = { x = 346.52, y = -1013.19, z = -99.2, h = 357.81 },  -- Inside the house coords
    animPos = { x = 76.52 , y = -1948.16 , z = 21.17, h = 21.17 }, -- The animation position
    doorTime = {}
   }
}
...
[" Mesa da cozinha encontraste "] = { x = 342.23, y = -1003.29, z = -99.0,  amount = 0},
[" Gaveta da tv encontraste "] = { x = 338.14, y = -997.69,  z = -99.2,  amount = 0},
[" Gaveta do quarto encontraste "] = { x = 350.91, y = -999.26,  z = -99.2,  amount = 0},
[" Gaveta junto a cama encontraste "] = { x = 349.19, y = -994.83,  z = -99.2,  amount = 0},
[" Prateleira dos livros encontraste"] = { x = 345.3,  y = -995.76,  z = -99.2,  amount = 0},
[" Mesa do corredor encontraste "] = { x = 346.14, y = -1001.55, z = -99.2,  amount = 0},
[" Mesa da casa de banho encontraste "] = { x = 347.23, y = -994.09,  z = -99.2,  amount = 0},
[" Mesa de jantar encontraste "] = { x = 339.23, y = -1003.35, z = -99.2,  amount = 0},
[" Roupas encontraste "] = { x = 351.24, y = -993.53,  z = -99.2,  amount = 0}
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(120000)
		ESX.TriggerServerCallback("99kr-burglary:checkCops",function(cb)
			copsOnline = cb
		end)
	end
end)
Citizen.CreateThread(function()
	while true do
      ...
      if GetClockHours() >= 0  and GetClockHours() <= 8 then
        if dist <= 1.2 and lockpicking == false then
          vf = v.locked
        end
            ...
            DrawText3D(v.pos.x, v.pos.y, v.pos.z, text, 0.4)
						...
						Citizen.Wait(5000)
						ESX.ShowNotification("Não tem policias suficientes online!")
					end	
				else
					if dist <= 1.2 and timer == true then
            local secsRemaining = math.ceil((v.doorTime - GetGameTimer()) / 1200)
						secsRemaining = secsRemaining - 1
						if secsRemaining > 0 then
							DrawText3D(v.pos.x, v.pos.y, v.pos.z,'Por favor espera ~r~'..secsRemaining..'~w~ até que possas voltar a forçar a fechadura', 0.4)
						else
							timer = false
							v.locked = true 
							doorTime = {}      
				...
				if dist <= 1.2 then
					breakTime = 24 - GetClockHours() 
					DrawText3D(v.pos.x, v.pos.y, v.pos.z, 'Podes roubar a casa em ~r~' ..breakTime.. ' ~w~horas', 0.4) 
				end
			end
		end
	end
...
function confMenu(house)
  Citizen.Wait(6)
  RemoveResidents()
  v = GetHouseValues(house, burglaryPlaces)
...
RegisterNetEvent('99kr-burglary:stopanimlose')
AddEventHandler('99kr-burglary:stopanimlose', function()
  local playerPed = PlayerPedId()
  lockpicking = false
  ClearPedTasks(playerPed)
  FreezeEntityPosition(playerPed, false)
...
local obj1 = nil
local obj2 = nil
RegisterNetEvent('99kr-burglary:stopanimwon')
AddEventHandler('99kr-burglary:stopanimwon', function()
  local playerPed = PlayerPedId()
  lockpicking = false
  v.locked = false
  ClearPedTasks(playerPed)
...
function DrawNoiseBar(noise, barIndex)
	DrawTimerBar("RUIDO", math.floor(noise), barIndex)
end
...
function callCops(house)
	...
	TriggerEvent('skinchanger:getSkin', function(skin)
        TriggerServerEvent('esx-qalle-camerasystem:addWitness', skin, "Assalto a Casa")
    end)
  --  end
end
...
Citizen.CreateThread(function(house)
	while true do
    Citizen.Wait(6)
    if GetPlayerCurrentStealthNoise(PlayerId()) > 3 then
...
function SpawnResidents(home)
		...
		RequestModel("u_m_o_tramp_01")
		while not HasModelLoaded("u_m_o_tramp_01") do 
		  Wait(0)
    end
...
local blips = {
  {title="Loja de penhores", colour=4, id=133, x = -2544.15, y = 2316.53, z = 33.22}
}
...
Citizen.CreateThread(function()
  while true do
      ...
      for k in pairs(gym) do
        ...
        if dist <= 0.5 then
          DrawText3D(plyCoords.x, plyCoords.y, plyCoords.z, "~w~Pressione ~r~[H] ~w~ para usar a loja!", 0.4)
...
function OpenSellMenu()
  ...
  ESX.UI.Menu.Open(
      ...
      {
          title    = 'Queres vender algo?',
          elements = {
              {label = 'Anel (€30)', value = 'ring'},
              {label = 'Rolex (€35)', value = 'rolex'},
              {label = 'Camera (€60)', value = 'camera'},
              {label = 'Colar de ouro (€55)', value = 'goldNecklace'},
              {label = 'Laptop (€90)', value = 'laptop'},
              {label = 'Samsung S10(€70)', value = 'samsungS10'},
          }
      },
      function(data, menu)
...
---
local objectTable
TriggerEvent('MF_SafeCracker:SpawnSafe',false,vector3(349.86, -1000.1, -88.01),3.31, function(callbackObjectTable)
...
AddEventHandler('MF_SafeCracker:EndMinigame', function(didWin)
  if didWin then
	roubarcofre = true
    TriggerServerEvent("kuana:abriucofrecasa")
  else
...
local roubarcofre = false
Citizen.CreateThread(function()
  while stealing == false do
    Citizen.Wait(5)
	if roubarcofre == false then
      local playerPed = PlayerPedId()
      local coords = GetEntityCoords(playerPed)
      local dist = GetDistanceBetweenCoords(349.79, -999.62, -99.2, coords.x, coords.y, coords.z, false)
      if dist <= 1.2 then
        DrawText3D(349.79, -999.62, -99.2, "~g~[E]~w~ Abrir o cofre", 0.4)
        if dist <= 0.5 and IsControlJustPressed(0, Keys["E"]) then
