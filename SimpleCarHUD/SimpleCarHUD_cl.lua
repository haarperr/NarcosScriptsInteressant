...
-- SEATBELT PARAMETERS
local seatbeltInput = 113                   -- Toggle seatbelt on/off with K
...
local active = true
RegisterNetEvent("simplecarhud:toogle")
AddEventHandler("simplecarhud:toogle",function(toogle)
	active = toogle
end)
-- Main thread
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if (IsPedInAnyVehicle(player, false)) and active then
            ...
            if GetIsVehicleEngineRunning(vehicle) and GetVehicleClass(vehicle) ~= 13 then
                ...
                drawTxt("COMBUSTIVEL", 2, fuelColorText, 0.4, screenPosX + 0.085, screenPosY + 0.018)
                ...
                drawTxt("CINTO", 2, seatbeltColor, 0.4, screenPosX + 0.080, screenPosY + 0.048)
            end
        else
