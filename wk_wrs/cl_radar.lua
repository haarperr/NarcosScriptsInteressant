    ...
    if ( IsPedSittingInAnyVehicle( ped ) ) then 
        ...
        if ( GetVehicleClass( vehicle ) == 18 ) then
            ...
            if ( radarEnabled ) then 
                Notify( "~b~Radar ligado." )
            else 
                Notify( "~b~Radar desligado." )
            end 
            ...
            Notify( "~r~Tens de estar dentro de um carro de policia." )
        end 
    else 
        Notify( "~r~Tens de estar dentro de um carro." )
    end 
end )
...
function ToggleSpeedType()
    if ( radarInfo.speedType == "mph" ) then 
        ...
        Notify( "~b~Velocidade Mudada para Kmh." )
    else 
        radarInfo.speedType = "mph"
        Notify( "~b~Velocidade Mudada para Mph." )
    end
end 
...
function ToggleLockBeep()
    if ( radarInfo.lockBeep ) then 
        ...
        Notify( "~b~Aviso de Excesso de velocidade Inativo." )
    else 
        radarInfo.lockBeep = true
        Notify( "~b~Aviso de Excesso de velocidade Ativo." )
    end    
end 
...
Citizen.CreateThread( function()
	while true do 
		...
		if ( IsDisabledControlPressed( 1, 36 ) and IsDisabledControlJustPressed( 1, 29 ) and IsPedSittingInAnyVehicle( ped ) ) then 
